...
async def lifespan(app: FastAPI):
    ...
    alembic.config.main(prog="alembic", argv=["-c",
    ...
    asyncio.create_task(
        redis_subscribe_worker_exceptions(get_settings().REDIS_EXCEPTIONS_CHANNEL))
...
async def repeat_measure_regular_metrics():
    await measure_regular_metrics(get_settings().DATABASE_PATH,
...
async def archive_sheets_cronjob(frequency: str, interval: int, current_time_unit: int):
    ...
    async with get_db_async() as db:
        sheets = await crud.get_sheets_by_id_hash(db, frequency, interval,
                                                  current_time_unit)
        for s in sheets:
            group_queue = await crud.get_group_priority_async(db, s.group_id)
            task = celery.signature("create_sheet_task", args=[
                schemas.SubmitSheet(sheet_id=s.id, author_id=s.author_id,
                                    group_id=s.group_id).model_dump_json()]).apply_async(
                **group_queue)
...
@repeat_every(seconds=DELETE_WINDOW, wait_first=180,
              on_exception=increase_exceptions_counter)
async def notify_about_expired_archives():
    ...
    if user_archives:
        ...
        for email in user_archives:
            list_of_archives = "\n".join(
                [f'{a.url}, {a.id}, {a.store_until.isoformat()}<br/>' for a in
                 user_archives[email]])
...
@repeat_every(max_repetitions=1, wait_first=10, seconds=0,
              on_exception=increase_exceptions_counter)
